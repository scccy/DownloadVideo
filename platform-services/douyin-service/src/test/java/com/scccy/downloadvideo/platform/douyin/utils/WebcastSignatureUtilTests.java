...
import com.scccy.downloadvideo.platform.douyin.model.entity.DouyinWebcastSignature;
...
class WebcastSignatureUtilTests {
    ...
    @Autowired
    private DouyinWebcastSignature webcastSignature;
    ...
    void shouldGenerateSignature() {
        ...
        try {
            // When
            log.info("开始生成签名...");
            webcastSignature.setRoomId(roomId)
                          .setUserUniqueId(userUniqueId)
                          .getResourcePath();

            // Then
            String xMsStub = webcastSignature.getXMsStub();
            String xBogus = webcastSignature.getXBogus();
    ...
    void shouldGenerateRawString() {
        ...
        // When
        String rawString = webcastSignature.getRawString();
